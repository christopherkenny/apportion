...
place_geometry <- function(geometry, position, scale = 1) {
  ...
  (geometry - sf::st_centroid(st_union(geometry))) * scale + sf::st_sfc(sf::st_point(position))
}
...
move_states <- function(shp, epsg = 'ESRI:102003', trim_hi = FALSE) {
  ...
  if (trim_hi) {
    ...
    us_hawaii_cong$geometry[
      !st_intersects(us_hawaii_cong, y = trim_to, sparse = FALSE)
    ] <- st_sfc(st_polygon())
